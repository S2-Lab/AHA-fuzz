...
public class ApeFuzzer {
  ...
  public static List<ApeEvent> generateFuzzingEvents() {
    ...
    while (repeat > 0) {
      ...
      switch (eventType) {
        case 0:
          Logger.iprintln(
            "Fuzzing: generate rotation events, but skip this time(BPFuzz)"
          );
  ...
  private static void generateClickEvent(
    List<ApeEvent> events,
    boolean longClick
  ) {
  ...
  protected static PointF randomPoint(int width, int height) {
    return new PointF(
      RandomHelper.nextInt(width),
      RandomHelper.nextInt(height)
    );
  }

  protected static PointF randomVector() {
    return new PointF(
      (RandomHelper.nextFloat() - 0.5f) * 50,
      (RandomHelper.nextFloat() - 0.5f) * 50
    );
  }

  protected static PointF randomWalk(
    int width,
    int height,
    PointF point,
    PointF vector
  ) {
    float x = (float) Math.max(
      Math.min(point.x + RandomHelper.nextFloat() * vector.x, width),
      0
    );
    float y = (float) Math.max(
      Math.min(point.y + RandomHelper.nextFloat() * vector.y, height),
      0
    );
  ...
  private static void generateTrackballEvent(List<ApeEvent> events) {
    ...
    events.add(
      new ApeTrackballEvent(deltaX, deltaY, RandomHelper.nextBoolean())
    );
  }
  ...
  private static void generateRotationEvent(List<ApeEvent> events) {
    int degree =
      SCREEN_ROTATION_DEGREES[RandomHelper.nextInt(
