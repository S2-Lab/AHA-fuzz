...
public class MonkeySourceRandom implements MonkeyEventSource {
  ...
  private static boolean validateKeyCategory(
    String catName,
    int[] keys,
    float factor
  ) {
    ...
    System.err.println(
      "** " + catName + " has no physical keys but with factor " + factor + "%."
    );
    return false;
  }
  ...
  public void setFactors(float factors[]) {
    ...
    for (int i = 0; i < c; i++) mFactors[i] = factors[i];
  }
  ...
  private PointF randomPoint(Random random, Display display) {
    return new PointF(
      random.nextInt(display.getWidth()),
  ...
  private void randomWalk(
    ...
    point.x =
      (float) Math.max(
        Math.min(point.x + random.nextFloat() * vector.x, display.getWidth()),
        0
      );
    point.y =
      (float) Math.max(
        Math.min(point.y + random.nextFloat() * vector.y, display.getHeight()),
        0
      );
  }
