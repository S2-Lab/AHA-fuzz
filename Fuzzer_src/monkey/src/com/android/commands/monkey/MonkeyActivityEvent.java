...
public class MonkeyActivityEvent extends MonkeyEvent {
  ...
  public ComponentName mApp;
  long mAlarmTime = 0;
  public HashMap<ComponentName, String> mMainApps = new HashMap<>();
  ...
  public int injectEvent(
    ...
    try {
      ...
      if (verbose > 0) {
        Logger.out.println(
          "** Permissions error starting activity " + intent.toUri(0)
        );
      }
  ...
  /**
   * Obtain the package name of the current process using the current UID. The
   * package name has to
   * match the current UID in IActivityManager#startActivityAsUser to be allowed
   * to start an
   * activity.
   */
